...
for timestep in progress:  #主訓練迴圈，進行TOTAL_TIMESTEPS次迭代
    ...
    while not done:
        ...
        custom_reward, distance = speed_reward(info, custom_reward, distance)
        custom_reward = final_flag_reward(info, custom_reward)
        custom_reward = score_reward(info, custom_reward, prev_info)
        custom_reward = altitude_reward(info, custom_reward)
        custom_reward = death_penalty(info, custom_reward, prev_info)
        custom_reward = alive_time_reward(info, custom_reward, prev_info)
