...
from gym_super_mario_bros.actions import CUSTOM_MOVEMENT         #從 gym_super_mario_bros中導入SIMPLE_MOVEMENT，這是一個預定義的按鈕動作集合（如「右移」、「跳躍」等），用於控制 Mario 的行為。
...
for timestep in tqdm(range(1, TOTAL_TIMESTEPS + 1), desc="Training Progress"):  #主訓練迴圈，進行TOTAL_TIMESTEPS次迭代
    ...
    while not done:
        ...
        custom_reward = distance_y_offset_reward(info, reward, prev_info)
        custom_reward = distance_x_offset_reward(info, reward, prev_info)
        custom_reward = final_flag_reward(info, reward)
        custom_reward = score_reward(info, reward, prev_info)
...
model_path = os.path.join(f"ckpt_test/{ROUND}",f"final_reward_{int(best_reward)}_custom_{int(cumulative_custom_reward)}.pth")
torch.save(dqn.q_net.state_dict(), model_path)
print(f"Model saved: {model_path}")
env.close()
